...
async def upload_game_report(
    ...
    svc = MatchService(db)
    try:
        created = await svc.create_from_save(
            raw, reporter_discord_id, is_cloud_game, discord_message_id
        )
        ...
        raise HTTPException(
            status_code=400, detail=f"Unrecognized save file format {exc}"
        ) from exc
...
async def append_message_id_list(
    payload: AppendDiscordMessageID = Form(), db=Depends(get_database)
):
    svc = MatchService(db)
    try:
        return await svc.append_discord_message_id_list(
            payload.match_id, payload.discord_message_id
        )
    except InvalidIDError as exc:
...
MATCH_UPDATE_SETTABLE_FIELDS = frozenset(
    {"players", "confirmed", "flagged", "flagged_by"}
)
...
async def set_player_order(payload: SetPlayerOrder = Form(), db=Depends(get_database)):
    svc = MatchService(db)
    try:
        return await svc.set_player_order(
            payload.match_id, payload.player_order, payload.discord_message_id
        )
    except InvalidIDError as exc:
        logger.error("🔴 Invalid match ID: %s", payload.match_id)
        raise HTTPException(status_code=400, detail="Invalid match ID") from exc
...
async def change_order(payload: ChangeOrder = Form(), db=Depends(get_database)):
    svc = MatchService(db)
    try:
        return await svc.change_order(
            payload.match_id, payload.new_order, payload.discord_message_id
        )
    except InvalidIDError as exc:
        logger.error("🔴 Invalid match ID: %s", payload.match_id)
        raise HTTPException(status_code=400, detail="Invalid match ID") from exc
...
async def delete_pending_match(
    payload: DeletePendingMatch = Form(), db=Depends(get_database)
):
    svc = MatchService(db)
    try:
...
async def trigger_quit(payload: TriggerQuit = Form(), db=Depends(get_database)):
    svc = MatchService(db)
    try:
        return await svc.trigger_quit(
            payload.match_id, payload.quitter_discord_id, payload.discord_message_id
        )
    except InvalidIDError as exc:
        logger.error(
            "🔴 Invalid quitter discord ID: %s, quitter_discord_id: %s",
            payload.match_id,
            payload.quitter_discord_id,
        )
        raise HTTPException(status_code=400, detail="Invalid match ID") from exc
    except NotFoundError as exc:
...
async def assign_discord_id(
    payload: AssignDiscordId = Form(), db=Depends(get_database)
):
    svc = MatchService(db)
    try:
        return await svc.assign_discord_id(
            payload.match_id,
            payload.player_id,
...
async def assign_discord_id_all(
    payload: AssignDiscordIdAll = Form(), db=Depends(get_database)
):
    svc = MatchService(db)
    try:
        return await svc.assign_discord_id_all(
            payload.match_id, payload.discord_id_list, payload.discord_message_id
        )
    except InvalidIDError as exc:
        logger.error(
            "🔴 Invalid player ID: %s, discord_id_list: %s",
            payload.match_id,
            payload.discord_id_list,
        )
        raise HTTPException(status_code=400, detail="Invalid player ID") from exc
    except NotFoundError as exc:
        logger.warning("🔴 Match not found. matchID: %s", payload.match_id)
...
async def remove_sub(payload: RemoveSub = Form(), db=Depends(get_database)):
    svc = MatchService(db)
    try:
        return await svc.remove_sub(
            payload.match_id, payload.sub_out_id, payload.discord_message_id
        )
    except InvalidIDError as exc:
        logger.error(
            "🔴 Invalid player ID: %s, sub_out_id: %s",
            payload.match_id,
            payload.sub_out_id,
        )
        raise HTTPException(status_code=400, detail="Invalid player ID") from exc
...
async def contest_report(
    payload: ContestReportRequest = Form(), db=Depends(get_database)
):
    svc = MatchService(db)
    try:
        return await svc.contest_report(
            payload.match_id,
            payload.contestor_discord_id,
            payload.reason,
            payload.discord_message_id,
        )
    except NotFoundError as exc:
...
@matches_router.put(
    "/get-leaderboard-ranking/", response_model=LeaderboardRankingResponse
)
