...
from app.models.schemas import MatchResponse, MatchUpdate, SetPlayerOrder, ChangeOrder, DeletePendingMatch, TriggerQuit, AppendDiscordMessageID, AssignDiscordId, AssignDiscordIdAll, AssignSub, RemoveSub, ApproveMatch, ContestReportRequest, RevertMatchRequest, LeaderboardRankingResponse
...
@router.put("/assign-discord-id-all/", response_model=MatchResponse)
async def assign_discord_id_all(payload: AssignDiscordIdAll = Form(), db = Depends(get_database)):
    svc = MatchService(db)
    match_id = payload.match_id
    discord_id_list = payload.discord_id_list
    discord_message_id = payload.discord_message_id
    try:
        return await svc.assign_discord_id_all(match_id, discord_id_list, discord_message_id)
    except InvalidIDError:
        logger.error(f"🔴 Invalid player ID: {match_id}, discord_id_list: {discord_id_list}")
        raise HTTPException(status_code=400, detail="Invalid player ID")
    except NotFoundError:
        logger.warning(f"🔴 Match not found. matchID: {match_id}")
        raise HTTPException(status_code=404, detail="Match not found")
...
@router.put("/revert-match/", response_model=MatchResponse)
async def contest_report(payload: RevertMatchRequest = Form(), db = Depends(get_database)):
    svc = MatchService(db)
    match_id = payload.match_id
    try:
